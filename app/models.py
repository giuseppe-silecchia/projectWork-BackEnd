...
class User(db.Model):
    ...
    firstName = db.Column(db.String(80), nullable=False)  # Nome dell'utente
    lastName = db.Column(db.String(80), nullable=False)  # Cognome dell'utente
...
class Booking(db.Model):
    ...
    room_id = db.Column(db.Integer, db.ForeignKey('room.id'),
                        nullable=False)  # Chiave esterna che collega la prenotazione a una stanza
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'),
                        nullable=False)  # Chiave esterna che collega la prenotazione a un utente
